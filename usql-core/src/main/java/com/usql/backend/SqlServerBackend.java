 ...
 */
public class SqlServerBackend implements DialectBackend {
    ...
    private String generateFunctionCall(IRFunctionCall fc, GenerateOptions opt) {
        List<String> argList = fc.args().stream().map(a -> generateExpr(a, opt)).collect(Collectors.toList());
        String argsStr = String.join(", ", argList);

        if (fc.keep() != null) return generateKeepPolyfill(fc, argsStr, opt);

        String result;
        if (functionCatalog != null) {
            var def = functionCatalog.get(fc.funcName());
            if (def.isPresent()) {
                var mapping = def.get().forDialect(Dialect.SQLSERVER);
