...
public class HandParser {
    ...
    SelectStmt parseSelect() {
        ...
        else if (is(TokenType.OFFSET) || is(TokenType.FETCH)) { Expression offset; if (is(TokenType.OFFSET)) { advance(); offset = parseExpr(); } else offset = new IntLiteral(0); if (is(TokenType.ROWS) || is(TokenType.ROW)) advance(); if (is(TokenType.FETCH)) advance(); if (is(TokenType.NEXT)) advance(); Expression limit = parseExpr(); if (is(TokenType.ROWS) || is(TokenType.ROW)) advance(); if (is(TokenType.ONLY)) advance(); fetch = new FetchClause(limit, offset); }
