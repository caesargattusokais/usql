...
public class HandParser {
    ...
    Statement parseAlter() { advance(); expect(TokenType.TABLE); String tbl = expectIdentifier(); if (is(TokenType.ADD)) { advance(); if (is(TokenType.COLUMN)) advance(); ColumnDef cd = parseColumnDef(); return new AlterTableStmt(tbl, new AddColumn(cd.name(), new DataTypeDecl(cd.typeName(), cd.typePrecision(), cd.typeScale()), cd.constraints(), cd.defaultValue())); } if (is(TokenType.DROP)) { advance(); if (is(TokenType.COLUMN)) advance(); return new AlterTableStmt(tbl, new DropColumn(expectIdentifier())); } if (is(TokenType.ALTER)) { advance(); if (is(TokenType.COLUMN)) advance(); String col = expectIdentifier(); if (is(TokenType.TYPE)) { advance(); return new AlterTableStmt(tbl, new AlterColumnType(col, parseDataTypeDecl())); } if (is(TokenType.SET)) { advance(); advance(); return new AlterTableStmt(tbl, new AlterColumnSetDefault(col, parseExpr())); } if (is(TokenType.DROP)) { advance(); return new AlterTableStmt(tbl, new AlterColumnDropDefault(col)); } } if (is(TokenType.RENAME)) { advance(); if (is(TokenType.COLUMN)) advance(); String old = expectIdentifier(); expect(TokenType.TO); return new AlterTableStmt(tbl, new RenameColumn(old, expectIdentifier())); } throw error("Unknown ALTER"); }
