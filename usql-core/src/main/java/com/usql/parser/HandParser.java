...
public class HandParser {
    ...
    CreateIndexStmt parseCreateIndex() { boolean uq = is(TokenType.UNIQUE); if (uq) advance(); expect(TokenType.INDEX); boolean ifn = false; if (is(TokenType.IF)) { advance(); advance(); ifn = true; } String nm = expect(TokenType.IDENTIFIER).text; expect(TokenType.ON); String tn = expect(TokenType.IDENTIFIER).text; expect(TokenType.LPAREN); List<IndexColumn> cs = new ArrayList<>(); do { String c = expect(TokenType.IDENTIFIER).text; boolean d = false; if (is(TokenType.DESC)) { advance(); d = true; } else if (is(TokenType.ASC)) advance(); cs.add(new IndexColumn(c, d, false)); } while (is(TokenType.COMMA) && advance() != null); expect(TokenType.RPAREN); Expression w = null; if (is(TokenType.WHERE)) { advance(); w = parseExpr(); } return new CreateIndexStmt(uq, ifn, nm, tn, cs, w); }

    List<ParamDef> parseParams() { expect(TokenType.LPAREN); List<ParamDef> p = new ArrayList<>(); if (!is(TokenType.RPAREN)) { do { ParamDir d = ParamDir.IN; if (is(TokenType.IN)) { advance(); } else if (is(TokenType.OUT)) { advance(); d = ParamDir.OUT; } else if (is(TokenType.INOUT)) { advance(); d = ParamDir.INOUT; } String n = expect(TokenType.IDENTIFIER).text; DataTypeDecl dt = parseDataTypeDecl(); p.add(new ParamDef(n, dt, d)); } while (is(TokenType.COMMA) && advance() != null); } expect(TokenType.RPAREN); return p; }
    ...
    Statement parseDrop() { advance(); if (is(TokenType.TABLE)) { advance(); boolean ifEx = false; if (is(TokenType.IF)) { advance(); advance(); ifEx = true; } String n = expect(TokenType.IDENTIFIER).text; boolean csc = false; if (is(TokenType.CASCADE) || is(TokenType.RESTRICT)) { advance(); csc = true; } return new DropTableStmt(n, ifEx, csc); } if (is(TokenType.INDEX)) { advance(); boolean ifEx = false; if (is(TokenType.IF)) { advance(); advance(); ifEx = true; } String n = expect(TokenType.IDENTIFIER).text; String t = null; if (is(TokenType.ON)) { advance(); t = expect(TokenType.IDENTIFIER).text; } return new DropIndexStmt(n, t, ifEx); } if (is(TokenType.DATABASE)) { advance(); boolean ifEx = false; if (is(TokenType.IF)) { advance(); advance(); ifEx = true; } return new DropDatabaseStmt(expect(TokenType.IDENTIFIER).text, ifEx); } throw error("Unknown DROP"); }

    Statement parseTruncate() { advance(); if (is(TokenType.TABLE)) advance(); return new TruncateStmt(expect(TokenType.IDENTIFIER).text); }

    Statement parseAlter() { advance(); expect(TokenType.TABLE); String tbl = expect(TokenType.IDENTIFIER).text; if (is(TokenType.ADD)) { advance(); if (is(TokenType.COLUMN)) advance(); ColumnDef cd = parseColumnDef(); return new AlterTableStmt(tbl, new AddColumn(cd.name(), new DataTypeDecl(cd.typeName(), cd.typePrecision(), cd.typeScale()), cd.constraints(), cd.defaultValue())); } if (is(TokenType.DROP)) { advance(); if (is(TokenType.COLUMN)) advance(); return new AlterTableStmt(tbl, new DropColumn(expect(TokenType.IDENTIFIER).text)); } if (is(TokenType.ALTER)) { advance(); if (is(TokenType.COLUMN)) advance(); String col = expect(TokenType.IDENTIFIER).text; if (is(TokenType.TYPE)) { advance(); return new AlterTableStmt(tbl, new AlterColumnType(col, parseDataTypeDecl())); } if (is(TokenType.SET)) { advance(); advance(); return new AlterTableStmt(tbl, new AlterColumnSetDefault(col, parseExpr())); } if (is(TokenType.DROP)) { advance(); return new AlterTableStmt(tbl, new AlterColumnDropDefault(col)); } } if (is(TokenType.RENAME)) { advance(); if (is(TokenType.COLUMN)) advance(); String old = expect(TokenType.IDENTIFIER).text; expect(TokenType.TO); return new AlterTableStmt(tbl, new RenameColumn(old, expect(TokenType.IDENTIFIER).text)); } throw error("Unknown ALTER"); }
    ...
    Expression parseIdentOrFunc() {
        ...
        if (is(TokenType.DOT)) { advance(); if (is(TokenType.STAR)) { advance(); return new StarExpr(name); } String col = expect(TokenType.IDENTIFIER).text; return new ColumnRef(List.of(name), col); }
