...
| Oracle | `"name"` | ROWNUM 包裹 | NUMBER(1) | SEQUENCE+TRIGGER | `\|\|` |
| 达梦 DM | `"name"` | `LIMIT/OFFSET` | BIT | IDENTITY | `\|\|` |
| SQL Server | `[name]` | `OFFSET/FETCH` | BIT | IDENTITY(1,1) | `+` |

### 基本语法差异

| U-SQL | MySQL | PostgreSQL | Oracle | 达梦 DM | SQL Server |
|-------|-------|-----------|--------|---------|------------|
| `AGG(x) KEEP (DENSE_RANK FIRST\|LAST ORDER BY y)` | 子查询+DENSE_RANK | 同 MySQL | 原生 KEEP | 同 MySQL | 同 MySQL |
| `SELECT expr` (无 FROM) | `SELECT expr` | `SELECT expr` | `SELECT expr FROM DUAL` | `SELECT expr FROM DUAL` | `SELECT expr` |
| `LENGTH(s)` | `LENGTH(s)` | `LENGTH(s)` | `LENGTH(s)` | `LENGTH(s)` | `LEN(s)` |
| `SUBSTR(s,start,len)` | `SUBSTR` | `SUBSTR` | `SUBSTR` | `SUBSTR` | `SUBSTRING` |
| `CEIL(n)` | `CEIL(n)` | `CEIL(n)` | `CEIL(n)` | `CEIL(n)` | `CEILING(n)` |
| `NVL(a,b)` | `IFNULL(a,b)` | `COALESCE(a,b)` | `NVL(a,b)` | `NVL(a,b)` | `ISNULL(a,b)` |
| `NOW()` | `NOW()` | `NOW()` | `SYSDATE` | `SYSDATE` | `GETDATE()` |
...
8. **KEEP 聚合使用标准语法**：`AGG(x) KEEP (DENSE_RANK FIRST|LAST ORDER BY y)`，Oracle 原生支持，其他方言自动翻译为子查询 + DENSE_RANK
9. **窗口函数使用标准 SQL 语法**：支持 `ROW_NUMBER`、`RANK`、`DENSE_RANK`、`LAG`、`LEAD`、`FIRST_VALUE`、`LAST_VALUE`、`NTILE`，窗口帧支持 `ROWS/RANGE BETWEEN ...`
...
SELECT RANK() OVER (PARTITION BY dept_id ORDER BY salary DESC) AS rk FROM employees
SELECT DENSE_RANK() OVER (PARTITION BY dept_id ORDER BY salary DESC) AS dr FROM employees
...
├── docker/         Docker Compose（MySQL/PG/Oracle/DM/SQL Server 五库环境）
