...
| `AGG(x) KEEP (DENSE_RANK FIRST\|LAST ORDER BY y)` | `FIRST_VALUE(x) OVER (ORDER BY y ROWS ...)` | `FIRST_VALUE(x) OVER (ORDER BY y ROWS ...)` | 原生 KEEP | `FIRST_VALUE(x) OVER (ORDER BY y ROWS ...)` |
